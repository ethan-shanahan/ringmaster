...
# COMPUTATIONS HENCEFORTH...
    ...
    def log(self, clear=False) -> None:
        if clear:
            ...
            self.grids.append(self.pg.copy())
            self.masks.append(self.mask.copy())
    
    def run(self, desired_stable_states: int) -> dict:
        start_time = time.process_time()
        results = {}
        state = 0  # * 0 == transient state, i == ith stable state
        while state <= desired_stable_states:
            self.log()
            if state == 0:
            ...
            results.update({'transient' if state == 0 
                      else f'stable_{state}':{
                                              'data':self.data.copy(), 
                                              'grid':self.pg.copy(), 
                                              'mask':self.mask.copy(), 
