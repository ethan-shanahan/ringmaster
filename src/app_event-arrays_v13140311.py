...
model     : str       = 'OFC'
output    : str       = 'perturbation event arrays'         # passed to CA, determines what gets recorded in Series
extract   : list[str] = ['natural_event']  # list of attributes of Series to be analysed
...
if file_type == 'read':
    ...
    for m in range(1, 1+machines):
        ...
        for ext in extracting:
            ...
            del data[0]['natural_event'][-1]
            # print(data)
            print('event durations :')
            event_size = 1 # min event duration
            event_this = []
            for i in range(len(next(iter(data[0].values())))):
                d = next(iter(data[0].values()))[i].array
                print(f'{len(d)}\t', end='', flush=True)
                if len(d) > event_size:
                    event_size = len(d)
                    event_this = d
            print(f'\n{event_size=}')
            final.extend(event_this)
            if len(final) == 0: 
                print('no large events'); exit()
