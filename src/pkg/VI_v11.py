...
class VisualInterface():
    ...
    def plotter(self, data : np.ndarray | list[np.ndarray], title : str, rows : int = 1, cols : int = 1, linear_scale : bool = False):
        ...
        ax.grid(); ax.legend()
        if linear_scale: ax.set_xscale('linear'); ax.set_yscale('linear')
