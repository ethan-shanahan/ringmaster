...
class CellularAutomaton():
    ...
    def __init__(
        ...
        self.data = np.empty((0,4), dtype=np.int8)
...
# COMPUTATIONS HENCEFORTH...
    ...
    def log(self, clear=False) -> None: # TODO rename to record, use log for a saved file of ex-terminal info
        if clear:
            ...
            self.data = np.concatenate((self.data, np.array([[self.time, self.energy, self.size, self.mass]])))
    ...
    def run(self, meta_time=None) -> dict:
        pbar = utils.ProgressBar(1, self.desired_stable_states+1, heading='seed', prefix=f'{self.seed} ', conclusion=f' {self.s+1}/{self.samples} samples computed ')
        ...
        while self.state <= self.desired_stable_states:
            ...
            self.data = pd.DataFrame(self.data, columns=['time', 'energy', 'size', 'mass'])
        ...
        if self.s+1 == self.samples:
            remaining = ''
        elif meta_time == 0:
            remaining = f'- approximately {round((self.comp_time["transient"]+self.comp_time["stable"]) * (self.samples-1))} seconds remaining '
            # print('meta_time is zero')
        elif meta_time is not None:
            remaining = f'- approximately {meta_time} seconds remaining '
            # print('meta_time is', meta_time)
        elif meta_time is None:
            remaining = ''
            # print('meta_time is None')
        pbar.update(self.state-1, conclusion_adjustment=remaining)
        # print(self.state-1, 'of', self.desired_stable_states)
        return results
